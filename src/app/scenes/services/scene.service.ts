...
import { ActionResponse } from 'src/app/devices/models/actionResponse.model';
...
export class SceneService {
  ...
  public setScene(scene: Scene): Observable<ActionResponse> {
    ...
    .pipe(map(scene => {
      this.notificationService.sendNotificationTemp(scene.status, scene.status === 'OK' ? 'success' : 'danger');
      return scene;
    }));
  }
}
